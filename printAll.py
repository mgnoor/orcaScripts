...
for i in range(2, len(sys.argv)):
	...
	for att in keys:
		attr = str(att)
		try:
			spline = romspline.readSpline(fileNames[i - 2], group=attr)
			...
			plt.savefig(name + "_" + attr + '.pdf')
		except:
			'''in my experience, trying to export these typically fails:
				CommonHorizonTimes
				HorizonATimes
				HorizonBTimes
				NRtimes
				auxiliary-info'''
			print 'Could not export ', attr
